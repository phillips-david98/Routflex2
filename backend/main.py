...
from models import SessionRegion, Customer, Vehicle, AdvancedPlanHistory
from schemas import (CustomerCreate, Customer, VehicleCreate, Vehicle,
                     SessionRegionCreate, SessionRegion, RouteReport,
...
@app.post("/sessions", response_model=SessionRegion)
def create_session(session: SessionRegionCreate, db: Session = Depends(get_db)):
    db_session = SessionRegion(code=session.code, name=session.name, ddd=session.ddd)
    db.add(db_session)
    db.commit()
...
def list_sessions(db: Session = Depends(get_db)):
    return db.query(SessionRegion).all()
...
def plan(
    ...
    if session_id is not None:
        customer_query = customer_query.filter(Customer.session_id == session_id)
...
@app.post("/customers/{customer_id}/visit-day")
...
def geocode_customer_address(customer_id: int, payload: CustomerGeocodingRequest, db: Session = Depends(get_db)):
    """
    ...
    result = geocoding_svc.geocode(
        customer_id=customer_id,
        address=payload.address or customer.address,
        number=payload.number or customer.number,
        neighborhood=payload.neighborhood or customer.neighborhood,
        city=payload.city or customer.city,
        state=payload.state or customer.state,
        user=payload.user,
    )
    ...
    if result.get("success"):
        ...
        customer.status = ClientStatus.ATIVO
        db.add(customer)
        db.commit()
        db.refresh(customer)
